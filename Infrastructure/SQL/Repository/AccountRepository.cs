...
namespace Infrastructure.SQL.Repository
{
    public class AccountRepository : IAccountRepository
    {
        ...
        {
            var user = await _context.persons.Include(p => p.Role).Include(p => p.tokens).SingleOrDefaultAsync(p => p.Id == Id);
            return user;
        }
        private async Task<Person> GetPersonByInfo(string Name, int RoleId, string HashedPassword)
        {
            var person = await _context.persons.Include(p => p.Role).SingleOrDefaultAsync(p => p.Name == Name && p.Role.Id == RoleId && p.PasswordHash == HashedPassword);
            return person;
        }

        private async Task<Person> ValidatePerson(LoginDTO user)
        {
            var Person = await _context.persons.FirstOrDefaultAsync(x => x.Name == user.Name && x.PasswordHash == Getsha256Hash(user.Password));
            if (Person != null)
            {
                return Person;
            }
        ...
        private async Task<int> AddToken(int PersonId, TokenResponseDTO Token)
        {
            var token = new Token()
            {
                ...
                RefreshTokenExp = DateTime.Now.AddMinutes(int.Parse(_configuration["JWtConfig:RefreshTokenExpires"])),
        ...
        private async Task<TokenResponseDTO> GenerateToken(int PersonId)
        {
            ...
            {
                new Claim(ClaimTypes.NameIdentifier, Person.Id.ToString() ?? throw new InvalidOperationException()),
